...
struct DPP_EXPORT select_option {
	...
	struct inner_select_emoji {
		 ...
		 * as with other emojis, that the shard must be on
...
struct DPP_EXPORT reaction {
	...
	uint32_t count;
	/** Reaction was from the shard's id */
	bool me;
...
enum message_flags {
	...
	m_ephemeral = 1 << 6,
	/// this message is an Interaction Response and the shard is "thinking"
	m_loading = 1 << 7
};
...
enum cache_policy_setting_t {
	 ...
	 * This is the default behaviour and the least memory-efficient option. GuildMemory usage will increase
	 * over time, initially quite rapidly, and then linearly over time. It is the least cpu-intensive
	 * setting.
	 */
	cp_aggressive = 0,
	/**
	 * @brief only cache when there is relevant activity, e.g. a message to the shard.
