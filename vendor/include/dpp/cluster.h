...
/**
 * @brief Represents the various information from the 'get gateway shard' api call
 */
...
class DPP_EXPORT cluster {
	...
	/**
	 * @brief Active DM channels for the shard
	 */
...
public:
	/**
	 * @brief Current shard token for all shards on this cluster and all commands sent via HTTP
	 */
	std::string token;

	/**
	 * @brief Last time the shard sent an IDENTIFY
	 */
	...
	/**
	 * @brief The details of the shard user. This is assumed to be identical across all shards
	 ...
	 * @param token The shard token to use for all HTTP commands and websocket connections
	 ...
	 * @param shards The total number of shards on this shard. If there are multiple clusters, then (shards / clusters) actual shards will run on this cluster.
	 ...
	 * @param policy Set the user caching policy for the cluster, either lazy (only cache users/members when they message the shard) or aggressive (request whole member lists on seeing new guilds too)
	 ...
	 * Example:
	 * ```
	 * shard.set_audit_reason("Too much abusive content")
	 ...
	 * Example:
	 * ```
	 * shard.set_audit_reason("Won't be sent")
	 *	.clear_audit_reason()
	 *	.channel_delete(my_channel_id);
	 * ```
	 * 
	 ...
	 *
	 * @param return_after If true the shard will return to your program after starting shards, if false this function will never return.
	 */
	 ...
	 * A guild can be deleted via the shard being kicked, the shard leaving the guild
	 ...
	 * increase your shard's CPU usage. If you don't need them it is recommended to not ask
	 * for them.
	 *
	 * @note Use operator() to attach a lambda to this event, and the detach method to detach the listener using the returned ID.
	 ...
	 * should create something yourself within your shard. Caching of messages is not on
	 ...
	 * by the shard developer. Each track marker can have a string value associated with it
	...
	/**
	 * @brief Create a global slash command (a shard can have a maximum of 100 of these).
	 * 
	...
	/**
	 * @brief Edit a global slash command (a shard can have a maximum of 100 of these)
	 *
	...
	/**
	 * @brief Delete a global slash command (a shard can have a maximum of 100 of these)
	 *
	 ...
	 * only permissions your shard has in the guild can be allowed/denied. Setting `MANAGE_ROLES` permission in channels is only possible
	 ...
	 * The shard must be a member of the guild with `CREATE_INSTANT_INVITE` permission.
	 ...
	 * 
	 * Delete the attached integration object for the guild. Deletes any associated webhooks and kicks the associated shard if there is one.
	...
	/**
	 * @brief Get current (shard) application
	 *
	 * @see https://discord.com/developers/docs/topics/oauth2#get-current-shard-application-information
	 * @param callback Function to call when the API call completes.
	...
	/**
	 * @brief Get current (shard) user guilds
	...
	/**
	 * @brief Get the gateway information for the shard using the token
	 * @see https://discord.com/developers/docs/topics/gateway#get-gateway-shard
	 * @param callback Function to call when the API call completes.
	...
	/**
	 * @brief Set the shard's voice state on a stage channel
	 * 
	 * **Caveats**
	 * 
	 ...
	 * - When unsuppressed, non-shard users will have their `request_to_speak_timestamp` set to the current time. Bot users will not.
