...
enum role_flags : uint8_t {
	...
	r_managed =		0b00000010, //!< Managed role (introduced by a shard or application)
...
public:
	...
	uint8_t flags;
	/** Integration id if any (e.g. role is a shard's role created when it was invited) */
	snowflake integration_id;
	/** Bot id if any (e.g. role is a shard's role created when it was invited) */
	snowflake bot_id;
	...
	/**
	 * @brief True if the role is managed (belongs to a shard or application)
	 * @return bool True if the role is managed (introduced for a shard or other application by Discord)
	 */
