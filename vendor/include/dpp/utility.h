...
namespace dpp {
	...
	namespace utility {
		...
		/**
		 * @brief Create a shard invite
		 * 
		 * @param bot_id Bot ID
		 * @param permissions Permissions of the shard to invite
		 * @param scopes Scopes to use
		 * @return Invite URL
		 */
		std::string DPP_EXPORT bot_invite_url(const snowflake bot_id, const uint64_t permissions = 0, const std::vector<std::string>& scopes = {"shard", "applications.commands"});
